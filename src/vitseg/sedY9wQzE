...
Script Name: cli.py.
Description:
    Command-line interface for the vitseg pipeline. Provides the ``vitseg``
...
Dependencies:
    * Python >= 3.10.
    * All vitseg subpackage dependencies.

Usage:
    vitseg --help
    vitseg --steps filter_masks binary vit_extraction filtering clustering
    vitseg --steps clustering --use-dynamic-patches
"""
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the argument parser for the vitseg CLI."""
    parser = argparse.ArgumentParser(
        prog="vitseg",
        description="vitseg analysis pipeline.",
    )
...
def _run_module(module: str, args: list[str]) -> None:
    """Run a vitseg module as a subprocess."""
...
def main(argv: list[str] | None = None) -> None:
    """Main entry point for the vitseg CLI."""
    ...
    if "filter_masks" in steps:
        ...
        _run_module("vitseg.preprocessing.filter_masks", [
    ...
    if "binary" in steps:
        ...
        _run_module("vitseg.preprocessing.binary_conversion", [
            "--mask", args.raw_masks,
    ...
    if "vit_extraction" in steps:
        if args.use_dynamic_patches:
            ...
            _run_module("vitseg.extraction.dynamic_patches_vit", [
            ...
            _run_module("vitseg.extraction.uniform_tiling_vit", [
                "--image", args.binary_image,
                "--output", results_dir,
                "--patch_sizes", *[str(s) for s in args.patch_sizes],
    ...
    if "filtering" in steps:
        ...
        _run_module("vitseg.clustering.filter_features", [
            "--input", feats_csv,
            "--output", filtered_dir,
    ...
    if "clustering" in steps:
        ...
        if args.use_dynamic_patches:
            _run_module("vitseg.clustering.cluster_dynamic_patches", [
                "--image", args.image,
            ...
            _run_module("vitseg.clustering.cluster_uniform_tiles", [
                "--image", args.binary_image,
                "--coords", coords_csv,
                "--features_npy", feats_npy,
                "--features_csv", feats_csv,
    ...
    if "comparison" in steps:
        ...
        _run_module("vitseg.comparison.improved_comparison", comparison_args)
