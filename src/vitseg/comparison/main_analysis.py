...
Dependencies:
    • Python >= 3.10.
    • All modules in comparison_analysis/scripts/.
...
def run_subprocess_analysis(script_path: Path, args: List[str], description: str) -> bool:
    """
    Run a subprocess analysis script with error handling.
    
    Args:
        script_path: Path to the Python script to execute.
        args: List of command-line arguments for the script.
        description: Description of the analysis for progress tracking.
        
    Returns:
        Boolean indicating success or failure.
        
    This function provides robust subprocess execution with proper
...
def run_complete_analysis(vit_clusters: Path, spatial_clusters: Path, output_dirs: Dict[str, Path],
    ...
    results['metrics'] = run_subprocess_analysis(
        scripts_dir / 'cluster_metrics.py',
        metrics_args,
    ...
    results['spatial'] = run_subprocess_analysis(
        scripts_dir / 'spatial_analysis.py',
        spatial_args,
    ...
    if results['metrics']:
        ...
        results['visualizations'] = run_subprocess_analysis(
            scripts_dir / 'visualization_suite.py',
            viz_args,
