...
Dependencies:
    • Python >= 3.10.
    • All modules in vigseg.comparison.
    • pandas, numpy, scipy, scikit-learn.
    • rich (for enhanced console output).

Usage:
    python -m vigseg.comparison.main_analysis \
...
def run_subprocess_analysis(module_name: str, args: List[str], description: str) -> bool:
    ...
    Args:
        module_name (str): Fully qualified module name (e.g. 'vigseg.comparison.cluster_metrics').
...
def run_complete_analysis(vit_clusters: Path, spatial_clusters: Path, output_dirs: Dict[str, Path],
    ...
    results['metrics'] = run_subprocess_analysis(
        'vigseg.comparison.cluster_metrics',
        metrics_args,
    ...
    results['spatial'] = run_subprocess_analysis(
        'vigseg.comparison.spatial_analysis',
        spatial_args,
    ...
    if results['metrics']:
        ...
        results['visualizations'] = run_subprocess_analysis(
            'vigseg.comparison.visualization_suite',
            viz_args,
